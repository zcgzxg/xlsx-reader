...
export function getStringFromBytes(
  ...
  for (; byteOffset < byteLength; byteOffset++) {
    ...
    if (byteOffset + charLength > byteLength) {
      ...
      chars.push(String.fromCodePoint(getCharCode(bytes, byteOffset, charLength)))
    }
...
export function setBytesFromString(
  ...
  for (let i = 0, j = string.length; i < j; i++) {
    ...
    if (strict && byteOffset + neededBytes > byteLength) {
      throw new Error('E_BUFFER_OVERFLOW' + '-' + string[i] + '-' + byteOffset)
    }
    setBytesFromCharCode(string[i].codePointAt(0) as number, bytes, byteOffset, neededBytes)
